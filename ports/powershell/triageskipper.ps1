...
function Check-Keyboard {
    $index = 0
    try {
        $devices = Get-CimInstance -ClassName Win32_Keyboard -ErrorAction Stop
...
function Check-Mouse {
    $index = 0
    try {
        $devices = Get-CimInstance -ClassName Win32_PointingDevice -ErrorAction Stop
        foreach ($obj in $devices) {
            $description = [string]($obj.Description)
...
function Check-Monitor {
    $index = 0
    try {
        $devices = Get-CimInstance -ClassName Win32_DesktopMonitor -ErrorAction Stop
        foreach ($obj in $devices) {
            $description = [string]($obj.Description)
            $pnpDeviceId = [string]($obj.PNPDeviceID)
